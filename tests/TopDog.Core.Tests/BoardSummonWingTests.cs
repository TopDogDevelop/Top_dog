...
public sealed class BoardSummonWingTests
{
    ...
    public void LiveCombat_Summon_SpawnsFiveWingsFromCaster()
    {
        ...
        var dreadCount = ships.AllHulls().Count(h =>
            "DREADNOUGHT".Equals(h.tonnageClass, StringComparison.OrdinalIgnoreCase));
        Assert.That(dreadCount, Is.GreaterThan(0), "content/ships 需含 DREADNOUGHT hull");

        var echo = BoardSummonWingService.TrySpawnFromCaster(
            state, bf, caster, ships, ModuleRegistry.LoadDefault(), new Random(1));
    ...
    public void CapFull_RejectsSpawn()
    {
        ...
        bf.units.Add(caster);
        var spawned = BoardSummonWingService.SpawnFromCasterUnit(
            bf, caster, ShipRegistry.LoadDefault(), ModuleRegistry.LoadDefault(), new Random(1));
