 ...
 * · sessionSeed：开局一次真随机盐（Unity 兼容，无 Random.Shared）；回合刷新 mix(storyRound, gameWeek)
...
// liketocoode3a5
{
    ...
    {
        var rng = CreateRefreshRng(state);
    ...
    private static readonly object SessionSeedLock = new();
    private static int _sessionSeedSeq;

    internal static void EnsureSessionSeed(GameState state)
    {
        if (state.market.sessionSeed != 0)
        {
            return;
        }

        lock (SessionSeedLock)
        {
            if (state.market.sessionSeed != 0)
            {
                return;
            ...
            var seq = ++_sessionSeedSeq;
            var mixed = unchecked(
                Environment.TickCount * 7919
                ^ seq * 31
                ^ Guid.NewGuid().GetHashCode());
            state.market.sessionSeed = mixed == 0 ? 1 : mixed;
        }
    }

    internal static Random CreateRefreshRng(GameState state)
    {
        EnsureSessionSeed(state);
        var mixed = unchecked(state.market.sessionSeed * 397 ^ state.storyRound * 31 ^ state.gameWeek);
